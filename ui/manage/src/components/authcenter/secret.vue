    ...
    <el-drawer v-model="state.isShow" title="密钥管理" :before-close="handleClose" size="35%">
        <div class="drawer__content">
            <el-form :model="state.ruleForm" ref="ruleFormRef">
                <el-form-item label="密钥类型" label-width="80px" prop="type"
                    :rules="[{ required: true, message: '请选择密钥类型' }]">
                    <el-select v-model="state.ruleForm.type" placeholder="" filterable @change="">
                        <el-option v-for="item in state.secretTypes" :key="item" :label="item" :value="item">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="哈希类型" label-width="80px" prop="hashType"
                    v-show="state.ruleForm.type == 'SharedSecret'">
                    <el-select v-model="state.ruleForm.hashType" placeholder="" filterable @change="">
                        <el-option v-for="item in state.hashTypes" :key="item.value" :label="item.text"
                            :value="item.value">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label="密钥值" label-width="80px" prop="value"
                    :rules="[{ required: true, message: '请填写密钥值' }]">
                    <el-input v-model="state.ruleForm.value" autocomplete="off" />
                </el-form-item>
                <el-form-item label="到期时间" label-width="80px" prop="value">
                    <el-date-picker v-model="state.ruleForm.expiration" type="date" :editable="false"
...
import { getHashTypes, getSecretTypes } from '/@/api/config';
import { secretDto } from '/@/types/api/authcenter/secret';
