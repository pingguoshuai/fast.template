<template>
    <div class="system-scope-container">
            ...
            <el-table :data="state.resultDto.items">
                ...
                <el-table-column label="操作" width="280">
                    <template #default="scope">
                        ...
                        <el-button size="small" text type="primary" @click="openCliam(scope.row.id)">
                            管理声明</el-button>
        ...
        <ClaimForm ref="claimRef" loadUrl="/api/IdsAdmin/client/{0}/claims" postUrl="/api/IdsAdmin/client/{0}/add-claim"
            deleteUrl="/api/IdsAdmin/client/{0}/remove-claim?type={1}" />
        <!-- <EditForm ref="editRef" /> -->
        <ManageProperty ref="propertyRef" loadUrl="/api/IdsAdmin/client/{0}/properties"
            postUrl="/api/IdsAdmin/client/{0}/add-propery"
            deleteUrl="/api/IdsAdmin/client/{0}/remove-propery?key={1}" />

        <ManageSecret ref="secretRef" loadUrl="/api/IdsAdmin/client/{0}/secrets"
...
import { clientDto } from '/@/types/api/authcenter/client';
...
async function getData(): Promise<void> {
    await apiService.getListAsync(state.queryParam)
        .then(res => {
