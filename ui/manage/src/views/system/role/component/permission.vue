<template>
	<el-dialog :title="state.title" v-model="state.isShow" width="70%" :before-close="closeDialog">
		...
		<el-tabs v-model="activeName" type="card" tab-position="left" @tab-click="tabChange" class="demo-tabs">
			<el-tab-pane v-for="group in state.permissionDto.groups" :key="group.displayNameKey" :label="group.displayName" :name="group.displayNameKey">
				<el-scrollbar height="600px">
					...
					<el-row>
						<el-col v-for="item in group.permissions" :key="item.name" :class="{ pl30: item.parentName }" :span="getSpan(item)">
							<el-checkbox
								v-model="item.isGranted"
								:disabled="isGrantedByOtherProviderName(item)"
								:label="item.displayName"
								@change="checkboxChange(group, item, group.permissions)"
								size="large"
							/>
						</el-col>
					</el-row>
...
const updatePermissions = async () => {
	...
	await permissionService.updateAsync(state.currentProviderName, state.currnetProviderKey, permissions);
...
const getData = async (providerName: string, providerKey: string, displayName: string) => {
	const { data } = await permissionService.getAsync(providerName, providerKey);
