...
import { createRoleDto } from '/@/api/system/role/model';
...
const openEdit = async (id: string) => {
	...
	state.entityId = id;
	state.isShow = true;
	const { data } = await roleService.getAsync(id);
	state.formData = data;
};
...
const onSubmit = async () => {
	...
	if (state.isAdd) {
		await roleService.createAsync(state.formData);
	} else {
		await roleService.updateAsync(state.entityId, state.formData);
	}
	props.afterSubmit();
