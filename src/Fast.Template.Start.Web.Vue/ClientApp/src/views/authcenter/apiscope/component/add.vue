...
import { apiScopeCreate, apiScopeDetail, apiScopeUpdate } from '/@/api/authcenter/apiscope';
import { getStandardClaims } from '/@/api/config';
import { CreateApiScope } from '/@/types/api/authcenter/apiscope';
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const openEdit = async (id: string) => {
			...
			state.entityId = id;
			state.isShowDialog = true;
			const { data } = await apiScopeDetail(id);
			state.ruleForm = data;
		};
		...
		const closeDialog = async () => {
			...
			state.ruleForm = new CreateApiScope();
		};
		// 取消
		const onCancel = () => {
			handleClose();
		...
		const onSubmit = async () => {
			...
			if (state.isAdd) {
				await apiScopeCreate(state.ruleForm);
			} else {
				await apiScopeUpdate(state.entityId, state.ruleForm);
			}
			emit('reload');
