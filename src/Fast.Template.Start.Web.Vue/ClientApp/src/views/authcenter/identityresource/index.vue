    ...
    <div class="system-scope-container">
        <el-card shadow="hover">
            ...
            <el-pagination @size-change="onHandleSizeChange" @current-change="onHandleCurrentChange" class="mt15"
                :pager-count="5" :page-sizes="[2, 10, 20, 30]" v-model:current-page="state.queryParam.CurrentPage"
                background v-model:page-size="state.queryParam.maxResultCount"
                layout="total, sizes, prev, pager, next, jumper" :total="state.resultDto.totalCount">
            </el-pagination>
        </el-card>
        <AddForm ref="addRef" :after-submit="getData" />
        <ManageProperty ref="propertyRef" loadUrl="/api/IdsAdmin/identity-resource/{0}/properties"
            postUrl="/api/IdsAdmin/identity-resource/{0}/add-propery"
            deleteUrl="/api/IdsAdmin/identity-resource/{0}/remove-propery?key={1}" />
    </div>
</template>
...
import { identityResourceDto } from '/@/types/api/authcenter/identityresource';
...
const state = reactive({
    ...
    resultDto: new PageResult<identityResourceDto>()
});
const identityResourceService = new identityResource();

async function getData(): Promise<void> {
    await identityResourceService.getListAsync(state.queryParam)
        .then(res => {
...
const onOpenEdit = (row: identityResourceDto) => {
