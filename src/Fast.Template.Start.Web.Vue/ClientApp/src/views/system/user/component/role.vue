...
import { identityUserUpdateRolesDto } from '/@/types/api/system/user/identityuser';
...
const open = async (id: string) => {
	state.isShow = true;
	state.entityId = id;
	const { data } = await apiService.getRolesAsync(id);
	let roles: string[] = [];
...
const onSubmit = async () => {
	...
	await apiService.updateRolesAsync(state.entityId, state.formData);
...
onMounted(async () => {
	const { data } = await apiService.getAssignableRolesAsync();
